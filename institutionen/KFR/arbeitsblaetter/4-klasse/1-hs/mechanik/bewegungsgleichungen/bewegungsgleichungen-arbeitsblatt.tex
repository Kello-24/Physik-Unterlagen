...
% Recap der nötigen Werte. Bewusst noch mit konstanten Geschwindigkeiten
% (keine Beschleunigung) -- das ist die Grundlage, auf die die spätere
% Übung "Überholmanöver mit Beschleunigung" weiter unten aufbaut.
\begin{questions}
\begin{taskbox}[Übung: Überholmanöver auf der Autobahn]
\question[8] Wiederholung aus dem $v$-$t$-Diagrammarbeitsblatt: Auf der
...
\begin{parts}
  ...
  \part[5] Für das eigentliche Überholmanöver (auf der Überholspur)
  ...
  es wieder auf die rechte Spur einschwenken kann. Wie schnell muss das
  überholende Auto während des Manövers mindestens fahren, wenn der
  Lastwagen dabei unverändert mit $v_2$ weiterfährt? Gib das Ergebnis in
  $\si{\meter\per\second}$ \emph{und} in $\si{\kilo\meter\per\hour}$ an.
  \Antwortraum{5}
  \begin{solution}
  ...
  \[
    \Delta v = \frac{\SI{86.5}{\meter}}{\SI{6}{\second}}
    \approx \SI{14.42}{\meter\per\second}.
  \]
  Die nötige Geschwindigkeit des überholenden Autos:
  \[
    v_{\text{überhol}} = v_2 + \Delta v \approx \SI{22.22}{\meter\per\second}
    + \SI{14.42}{\meter\per\second} = \SI{36.64}{\meter\per\second}
    \approx \SI{131.9}{\kilo\meter\per\hour}.
  \]
  \textbf{Hinweis:} Das liegt noch deutlicher über der zulässigen
  Autobahnhöchstgeschwindigkeit von $\SI{120}{\kilo\meter\per\hour}$ als
  ohne Berücksichtigung der Lastwagenlänge. Dieses Überholmanöver ist im
  gegebenen Zeitfenster also erst recht nicht regelkonform durchführbar,
  das Auto müsste dafür deutlich zu schnell fahren.
  \end{solution}
\end{parts}
\end{taskbox}
\end{questions}
...
\begin{parts}
  ...
  Bremsstrecke aus dem Beispiel oben ($\Delta s=\SI{9}{\meter}$) über

...
% massstabsgetreu einzeichnen (Teil b), dann darauf aufbauend eine
% Anwendung mit Relativgeschwindigkeit (Teile c/d, Überholmanöver auf der
% Autobahn), bewusst als Vertiefung/Transfer über die Theoriebox hinaus,
% keine eigene Lernzielnummer. Werte bewusst so gewählt, dass $v_1$ genau
% der Autobahnhöchstgeschwindigkeit entspricht (120 km/h) und die in
% Teil d) errechnete nötige Überholgeschwindigkeit diese leicht
% überschreitet: Das zeigt, dass ein Überholmanöver bei Tempo 120 gegen
% einen Lastwagen (Tempo 80, die für Lastwagen geltende
% Autobahnhöchstgeschwindigkeit) im gegebenen Zeitfenster gar nicht
% regelkonform möglich ist. Bilder/Autos_Geschwindigkeit.png (leer, für
% Teil b) und Bilder/Autos_Geschwindigkeit_Loesung.png (mit
...
\question[13] Auf der Autobahn (Höchstgeschwindigkeit
$\SI{120}{\kilo\meter\per\hour}$) nähert sich ein Auto (im Bild links,
hinten) einem Lastwagen (im Bild rechts, vorne). Zum Zeitpunkt
$t=\SI{0}{\second}$ beträgt der Abstand zwischen den beiden Fahrzeugen
$d_0=\SI{70}{\meter}$ (siehe Bild).
...
\begin{parts}
  ...
  \part[4] Für das eigentliche Überholmanöver (auf der Überholspur)
  ...
  Lastwagen haben. Wie schnell muss das überholende Auto während des
  Manövers mindestens fahren, wenn der Lastwagen dabei unverändert mit
  $v_2$ weiterfährt? Gib das Ergebnis in $\si{\meter\per\second}$
  \emph{und} in $\si{\kilo\meter\per\hour}$ an.
  \Antwortraum{4}
  \begin{solution}
  Zu Beginn des Manövers liegt das Auto noch $\SI{40}{\meter}$ zurück, am
  Ende soll es $\SI{30}{\meter}$ Vorsprung haben: Relativ zum Lastwagen
  muss es also $\SI{40}{\meter}+\SI{30}{\meter}=\SI{70}{\meter}$
  aufholen, und zwar in $\SI{6}{\second}$:
  \[
    \Delta v = \frac{\SI{70}{\meter}}{\SI{6}{\second}}
    \approx \SI{11.67}{\meter\per\second}.
  \]
  Die nötige Geschwindigkeit des überholenden Autos:
  \[
    v_{\text{überhol}} = v_2 + \Delta v \approx \SI{22.22}{\meter\per\second}
    + \SI{11.67}{\meter\per\second} = \SI{33.89}{\meter\per\second}
    \approx \SI{122.0}{\kilo\meter\per\hour}.
  \]
  \textbf{Hinweis:} Das liegt über der zulässigen
  Autobahnhöchstgeschwindigkeit von $\SI{120}{\kilo\meter\per\hour}$.
  Dieses Überholmanöver ist im gegebenen Zeitfenster also gar nicht
  regelkonform durchführbar, das Auto müsste dafür zu schnell fahren.
  \end{solution}
\end{parts}
\end{taskbox}
\end{questions}

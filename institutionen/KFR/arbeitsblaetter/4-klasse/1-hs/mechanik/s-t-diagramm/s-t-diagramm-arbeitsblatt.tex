...
$s=\SI{40}{\kilo\meter}$ (gemessen ab einem festen Nullpunkt der
...
\begin{tikzpicture}[scale=1.0]
  ...
  \foreach \y/\lbl in {0/40,1/50,2/60,3/70} \draw (0.06,\y) -- (-0.06,\y) node[left] {\scriptsize \lbl};
  \draw[thick,titleblue] (0,0) -- (5,3);
  \filldraw[accentblue] (0,0) circle (1.6pt);
  \filldraw[accentblue] (5,3) circle (1.6pt);
\end{tikzpicture}

\footnotesize Start ($t=\SI{0}{\minute}$): $s=\SI{40}{\kilo\meter}$. \quad Ende ($t=\SI{15}{\minute}$): $s=\SI{70}{\kilo\meter}$.
\end{center}
\end{beispielbox}
...
\begin{parts}
  ...
  \]
  \end{solution}
  \part[2] Rechne dein Ergebnis aus Teilaufgabe (a) mithilfe von
  $1\,\si{\meter\per\second} = 3.6\,\si{\kilo\meter\per\hour}$ in
...
$t=\SI{36}{\minute}$: $s=\SI{48}{\kilo\meter}$ (Ausfahrt, Wende) \quad
...
\begin{parts}
  ...
  "typische" Momentangeschwindigkeit, sondern hängt nur von Start- und
  Endpunkt ab.
  \end{solution}
...
schnellste der drei; Fahrzeug 2 ist langsamer. Fahrzeug 1 bewegt sich
betragsmässig sogar am schnellsten (grösster Betrag der Steigung),
...
\begin{parts}
  ...
  \begin{tikzpicture}[scale=0.85]
    ...
    \foreach \x in {1,...,7} \draw (\x,0.08) -- (\x,-0.08) node[below] {\scriptsize \x};
    \foreach \y in {1,...,5} \draw (0.08,\y) -- (-0.08,\y) node[left] {\scriptsize \y};
    \draw[thick,titleblue] (0,0.6) -- (7,4.8) node[right] {\scriptsize Hauptfeld};
    \draw[thick,accentorange] (0,0) -- (7,4.9) node[right] {\scriptsize Favorit};
...
\begin{parts}
  ...
  \begin{tikzpicture}[scale=0.85]
    ...
    \draw[thick,accentorange] (0,9) -- (5,0) node[right] {\scriptsize Regionalexpress};

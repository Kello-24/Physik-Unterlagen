...
\begin{achtungbox}
\textbf{Temperatur und Wärme sind nicht dasselbe.} Ein grosser Topf und ein kleiner Becher mit demselben Wasser bei $80\,\celsius$ haben dieselbe \emph{Temperatur}. Der grosse Topf kann aber beim Abkühlen auf Zimmertemperatur eine viel grössere Wärmemenge abgeben, weil viel mehr Wasser (mehr Teilchen) abkühlt. Ein Körper „enthält“ also keine feste Menge Wärme, er kann nur beim Abkühlen unterschiedlich viel davon abgeben. Wer von „viel Wärme“ statt „hoher Temperatur“ spricht, meint physikalisch oft nicht dasselbe.
\end{achtungbox}
...
\]
Aufgelöst nach der Mischtemperatur (Ausmultiplizieren, Termen mit $\Temp_M$ auf eine Seite bringen, $\Temp_M$ ausklammern, durch die Klammer teilen):
\begin{align*}
  m_1c_1\Temp_1 - m_1c_1\Temp_M &= m_2c_2\Temp_M - m_2c_2\Temp_2 \\
  m_1c_1\Temp_1 + m_2c_2\Temp_2 &= m_2c_2\Temp_M + m_1c_1\Temp_M && \text{Terme mit } \Temp_M \text{ nach rechts, Rest nach links} \\
  m_1c_1\Temp_1 + m_2c_2\Temp_2 &= \Temp_M\,(m_1c_1+m_2c_2) && \text{$\Temp_M$ ausklammern} \\
  \Temp_M &= \frac{m_1c_1\Temp_1 + m_2c_2\Temp_2}{m_1c_1+m_2c_2} && \text{durch } (m_1c_1+m_2c_2) \text{ teilen}.
\end{align*}
...
Jede Messung hat eine begrenzte Genauigkeit: eine Waage zeigt nur auf ein Gramm genau an, ein Thermometer nur auf eine Gradteilung (mehr dazu in Abschnitt~4.3). Wird aus mehreren gemessenen Grössen eine neue Grösse berechnet (z.\,B.\ eine Dichte oder eine spezifische Wärmekapazität), so trägt das Ergebnis die Ungenauigkeiten aller einzelnen Messungen weiter. Die \textbf{\emph{Fehlerrechnung}} gibt an, wie ungenau das Endergebnis dadurch wird.
...
\begin{itemize}
  \item \textbf{Analoge Skala} (z.\,B.\ Thermometer, Massstab): $\Delta x$ entspricht meist der kleinsten Skalenteilung, oder der Hälfte davon, wenn man gut zwischen zwei Strichen interpolieren kann. Bei einem ruhenden Massstab funktioniert dieses Interpolieren meist gut. Ein Thermometer mit Strichen alle $1\ \mathrm{K}$ liefert dagegen $\Delta T\approx 1\ \mathrm{K}$: Es zeigt nur Unterschiede von $1\ \mathrm{K}$ an, ein zuverlässiges Interpolieren zwischen zwei Strichen ist hier nicht möglich.
